...
class HelzerAI:
    ...
    def instructions(self, guild):
        local = datetime.now(self.tz()).isoformat()
        return (
            "You are Helzer AI, a Discord administrator agent. Use tools for actions. "
            "Never invent IDs. Use mentioned user/channel IDs from context. Ask if ambiguous. "
            "Never claim success unless a tool reports success. High-risk tools use confirmation. "
            f"Server={guild.name} ({guild.id}); timezone={self.settings.ai_timezone}; local_time={local}. "
            "When the user says tomorrow/tonight/etc., calculate the timestamp using this timezone. "
            "Keep final responses concise."
        )

    async def handle_message(self, message):
        if not self.settings.ai_enabled or not self.client or not message.guild or message.author.bot:
            return
        if not isinstance(message.author, discord.Member) or not self.authorized(message.author):
            return
        if not self.bot.user or self.bot.user.mention not in message.content:
            return
        prompt=message.content.replace(self.bot.user.mention,"").strip()
        if not prompt:
            return
        try:
            view=await self.ask(message,prompt)
    ...
    async def ask(self,message,prompt):
        context=(f"Requester ID: {message.author.id}\nCurrent channel ID: {message.channel.id}\n"
                 f"Mentioned user IDs: {[u.id for u in message.mentions]}\n"
                 f"Mentioned channel IDs: {[c.id for c in message.channel_mentions]}\nRequest: {prompt}")
        r=await self.client.responses.create(model=self.settings.openai_model,instructions=self.instructions(message.guild),
                                             input=context,tools=TOOLS)
        for _ in range(8):
            calls=[x for x in r.output if getattr(x,"type",None)=="function_call"]
            if not calls:
                return result_view("Helzer AI", (r.output_text or "Done.").strip())
            outs=[]
            for call in calls:
            ...
            r=await self.client.responses.create(model=self.settings.openai_model,previous_response_id=r.id,input=outs,tools=TOOLS)
        return error_view("The request required too many steps.")
